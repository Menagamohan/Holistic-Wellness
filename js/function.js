(function ($) {
	...
	$("#enquiryForm").validator({ focus: false }).on("submit", function (event) {
		if(!event.isDefaultPrevented()) {
			event.preventDefault();
			submitForm("#enquiryForm","../booking-form.php",appointmentFormSuccess);
		}
	});
	...
	initSwiper(".testimonial-slider .swiper", {
		...swiperOptions,
		centeredSlides: true,
		pagination: {
	...
	/* Sisf-Swiper-Slider JS */
	initSwiper(".sisf-swiper-slider .swiper", {
		...swiperOptions,
		navigation: {
			nextEl: ".service-single-button-next",
			prevEl: ".service-single-button-prev"
